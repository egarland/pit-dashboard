...
import { HMAC_SHA1, PLAINTEXT } from "./sign";
...
const assertNotEquals = (actual: any, expected: any) => {
    expect(expected).not.to.eq(actual);
}
...
test("createBaseString - reproduces RFC example", () => {
...
test("createBaseString - reproduces Twitter example", () => {
...
test("OAuthClient.sign - generates unique nonce on each invocation", async () => {
...
test("OAuthClient.sign - uses current unix timestamp", async () => {
  const client = new OAuthClient({
    consumer: { key: "consumer-key", secret: "consumer-secret" },
    signature: PLAINTEXT,
  });
...
test("OAuthClient.sign - produces correct PLAINTEXT signature", async () => {
  const client = new OAuthClient({
    consumer: { key: "consumer-key", secret: "consumer-secret" },
    signature: PLAINTEXT,
  });
...
test("OAuthClient.sign - produces correct HMAC-SHA1 signature (RFC)", async () => {
...
// Taken from ddo/oauth-1.0a.
test("OAuthClient.sign - produces correct parameters (Twitter)", async () => {
  const client = new OAuthClient({
    consumer: {
...
// Taken from ddo/oauth-1.0a.
test("OAuthClient.sign - produces correct SHA1 body hash", async () => {
  const client = new OAuthClient({
    consumer: {
...
// Taken from ddo/oauth-1.0a.
test("OAuthClient.sign - computes correct signature for multi-valued body parameters", async () => {
  const client = new OAuthClient({
    consumer: {
...
// Taken from ddo/oauth-1.0a.
test("OAuthClient.sign - computes correct signature for multi-valued query parameters", async () => {
  const client = new OAuthClient({
    consumer: {
...
// Taken from ddo/oauth-1.0a.
test("toAuthHeader - produces correct header (Twitter)", () => {
...
test("toAuthHeader - produces correct header with realm (RFC)", () => {
  const actual = toAuthHeader(
    {
      oauth_consumer_key: "9djdj82h48djs9d2",
      oauth_token: "kkk9d7dh3k39sjv7",
...
test("toAuthHeader - escapes special characters in realm", () => {
  const actual = toAuthHeader(
    {
...
test("toQueryParams - produces correct query params (RFC)", () => {
  const actual = toQueryParams({
    ...
    oauth_nonce: "4572616e48616d6d65724c61686176", //@note: Hmm, these seem to be missing
    oauth_version: "1.0",
  });

  const expected = new URLSearchParams(
    "oauth_consumer_key=0685bd9184jfhq22&oauth_token=ad180jjd733klru7&" +
    "oauth_signature_method=HMAC-SHA1&oauth_signature=wOJIO9A2W5mFwDgiDvZbTSMK%2FPY%3D&oauth_timestamp=137131200&" +
      "oauth_nonce=4572616e48616d6d65724c61686176&oauth_version=1.0",
  );

  expect(Object.fromEntries(expected)).toEqual(Object.fromEntries(actual));
});
