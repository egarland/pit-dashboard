...
export {HMAC_SHA1, HMAC_SHA256, PLAINTEXT} from "../mod";
export type {OAuthOptions, SignatureMethod, Token} from "../mod";
...
export class Api {
  ...
  async request(
    ...
    const params = await this.client.sign(method, url.toString(), {
      token: opts?.token,
      params: opts?.params,
