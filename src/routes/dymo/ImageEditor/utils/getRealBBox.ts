...
const getRealBBox = async (obj: Fabric.ActiveSelection) => {

    let tempCanv: HTMLCanvasElement | null = null;
    let ctx: CanvasRenderingContext2D | null = null;
    let w: number, h: number;

    // we need to use a temp canvas to get imagedata
    const getImageData = (dataUrl: string):Promise<ArrayBuffer> => {
        if (tempCanv == null) {
    ...
    const scanPixels = (imageData: ArrayBuffer) => {
